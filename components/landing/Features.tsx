        ...
        >
          {features.map((feature, index) => (
            <motion.div
              ...
              className="h-full"
            >
              <Card className="h-full border border-gray-200 hover:shadow-xl transition-all duration-500 bg-white/80 backdrop-blur-sm hover:bg-white/90 group rounded-2xl">
                <CardContent className="p-6 sm:p-8 flex flex-col items-center text-center h-full">
                  {/* Icon */}
                  <motion.div
                    className={`w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-${feature.color}-100 to-${feature.color}-200 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-md group-hover:shadow-lg transition-all duration-300`}
                    whileHover={{ rotate: 360, scale: 1.1 }}
                    transition={{ duration: 0.6 }}
                  >
                    <feature.icon
                      className={`w-8 h-8 sm:w-10 sm:h-10 text-${feature.color}-600`}
                    />
                  </motion.div>

                  {/* Title */}
                  <h3
                    className={`text-lg sm:text-xl font-semibold text-${feature.color}-900 mb-3 sm:mb-4 font-serif`}
                  >
                    {feature.title}
                  </h3>
